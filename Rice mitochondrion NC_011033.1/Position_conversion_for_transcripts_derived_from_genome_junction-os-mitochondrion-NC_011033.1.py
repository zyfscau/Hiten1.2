...
# Step 1: Read input file and filter data based on conditions
selected_rows = []   # Rows meeting boundary conditions
unselected_rows = [] # Rows not meeting boundary conditions
...
with open(input_file, 'r') as f_in:
    ...
    for row in reader:
        ...
        col_5end = row[73]  # Column 74 (index 73) - 5' end position
        col_3end = row[74]  # Column 75 (index 74) - 3' end position
        try:
            op_val = float(row[75])  # Column 76 (index 75) - OP value
        except ValueError:
            op_val = 0.0
        col_strand = row[76]  # Column 77 (index 76) - strand direction
        
        # Check four boundary conditions
        condition1 = (col_5end == "490520") and (op_val < 0) and (col_strand == "-")
        condition2 = (col_5end == "1") and (op_val < 0) and (col_strand == "+")
        condition3 = (col_3end == "1") and (op_val < 0) and (col_strand == "-")
        condition4 = (col_3end == "490520") and (op_val < 0) and (col_strand == "+")
...
# Write unselected rows to high confidence transcripts file
with open(output_file1, 'w', newline='') as f_out1:
    writer = csv.writer(f_out1)
    writer.writerow(header)
    writer.writerows(unselected_rows)

# Write selected rows to junction-refinement file (raw data)
with open(output_file2, 'w', newline='') as f_out2:
    writer = csv.writer(f_out2)
    writer.writerow(header)
    writer.writerows(selected_rows)

# Step 3: Process selected rows - correct boundary position values
modified_rows = []
for row in selected_rows:
    ...
    if col_5end == "490520" and op_val < 0 and col_strand == "-":
        # Output absolute value of OP to 5'end (negative strand 5' end boundary correction)
        new_row[73] = str(int(abs(op_val)))  # Convert to integer
    elif col_3end == "490520" and op_val < 0 and col_strand == "+":
        # Output absolute value of OP to 3'end (positive strand 3' end boundary correction)
        new_row[74] = str(int(abs(op_val)))  # Convert to integer
    elif col_5end == "1" and op_val < 0 and col_strand == "+":
        # Calculate new value and convert to integer (positive strand 5' end boundary correction)
        new_value = 1 + 490520 + op_val
        new_row[73] = str(int(new_value))
    elif col_3end == "1" and op_val < 0 and col_strand == "-":
        # Calculate new value and convert to integer (negative strand 3' end boundary correction)
        new_value = 1 + 490520 + op_val
        new_row[74] = str(int(new_value))
    
    # Set OP to 0 (clear OP value after boundary correction)
    new_row[75] = "0"
    
    # Create connection string (MT|corrected_5end|corrected_3end|OP|strand)
    connected_str = f"MT|{new_row[73]}|{new_row[74]}|{new_row[75]}|{col_strand}"
    
    # Add to row as new column (column 78)
    new_row.append(connected_str)
    modified_rows.append(new_row)

# Step 4: Write modified data to junction-refinement-result file (with original columns + new column)
...
# Step 5: Replace values in first column with connection string
...
for row in modified_rows:
    ...
    # Add to final output list (keep only first 77 columns, remove Connected column)
    final_output_rows.append(new_row[:77])  # Keep only first 77 columns

# Step 6: Append processed data to high confidence transcripts file
with open(output_file1, 'a', newline='') as f_out1_append:
    writer = csv.writer(f_out1_append)
    
    # Skip header row, write only data rows
...
with open(output_file3, 'w', newline='') as f_out3:
    writer = csv.writer(f_out3)
    # Write new header (with Connected column)
    writer.writerow(new_header)
    
    # Write modified data (with all 78 columns)
    for row in modified_rows:
        # Create row copy
        new_row = row.copy()
